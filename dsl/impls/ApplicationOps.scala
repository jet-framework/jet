...
trait SimpleFileOps extends Base with Variables with OverloadHack with ParserOps  {
...
trait SimpleFileOpsExp extends SimpleFileOps with StructExp with EffectExp with BaseFatExp {
 def simplefile_obj_new(name: Exp[String], path: Exp[String], files: Exp[Array[SimpleFile]]) = struct[SimpleFile](ClassTag[SimpleFile]("SimpleFile"), ListMap( "name" -> name,  "path" -> path,  "files" -> files))
...
trait ApplicationOps extends SupplierOps with PartOps with CustomerOps with RegionOps with LineItemOps with WikiArticleOps with SimpleFileOps with AddressOps with PageCountEntryOps with N2Ops with PartSupplierOps with LogEntryOps with NationOps with UserOps with N1Ops with OrderOps
trait ApplicationOpsExp extends SupplierOpsExp with PartOpsExp with CustomerOpsExp with RegionOpsExp with LineItemOpsExp with WikiArticleOpsExp with SimpleFileOpsExp with AddressOpsExp with PageCountEntryOpsExp with N2OpsExp with PartSupplierOpsExp with LogEntryOpsExp with NationOpsExp with UserOpsExp with N1OpsExp with OrderOpsExp
